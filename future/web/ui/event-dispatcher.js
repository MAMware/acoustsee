...
export function createEventDispatcher(DOM) {
  ...
  if (!DOM) {
    ...
    return {
      dispatchEvent: () =>
        console.error("dispatchEvent not initialized due to undefined DOM"),
    };
  }
  ...
  const handlers = {
    updateUI: async ({ settingsMode, streamActive }) => {
      if (
        ...
        dispatchEvent("logError", {
          message: "Missing critical DOM elements for UI update",
        });
        return;
      }
      ...
      setTextAndAriaLabel(
        DOM.button2,
        settingsMode
          ? settings.synthesisEngine === "sine-wave"
            ? "Sine Wave"
            : "FM Synthesis"
          : streamActive
            ? "Audio Off"
            : "Audio On",
        settingsMode
          ? `Select synthesis: ${settings.synthesisEngine}`
          : `Toggle audio`,
      );
      ...
      await speak(settingsMode ? "viewDebug" : "saveSettings", {
        state: settingsMode ? "view" : "save",
      });
      setTextAndAriaLabel(
        DOM.button4,
      ...
      await speak(settingsMode ? "emailDebug" : "loadSettings", {
        state: settingsMode ? "email" : "load",
      });
      setTextAndAriaLabel(
        DOM.button5,
    ...
    processFrame: () => {
      try {
        ...
        dispatchEvent("logError", {
          message: `Process frame error: ${err.message}`,
        });
      }
    },
    ...
    saveSettings: async ({ settingsMode }) => {
      if (settingsMode) {
        ...
        try {
          ...
          dispatchEvent("logError", {
            message: `Save settings error: ${err.message}`,
          });
      ...
      if (settingsMode) {
        ...
        try {
          ...
          if (saved) {
            ...
            settings.synthesisEngine =
              parsed.synthesisEngine || settings.synthesisEngine;
            settings.language = parsed.language || settings.language;
            settings.autoFPS =
              parsed.autoFPS !== undefined ? parsed.autoFPS : settings.autoFPS;
            settings.updateInterval =
              parsed.updateInterval || settings.updateInterval;
          ...
          dispatchEvent("logError", {
            message: `Load settings error: ${err.message}`,
          });
    ...
    emailDebug: async () => {
      try {
        ...
        dispatchEvent("logError", {
          message: `Email debug error: ${err.message}`,
        });
  ...
  dispatchEvent = (eventName, payload = {}) => {
    if (handlers[eventName]) {
      try {
        ...
        handlers.logError({
          message: `Handler ${eventName} error: ${err.message}`,
        });
      }
    } else {
