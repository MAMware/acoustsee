...
export function setupRectangleHandlers() {
  ...
  if (
    ...
    dispatchEvent("logError", {
      message: "Critical DOM elements missing in rectangle-handlers",
    });
    return;
  }
    ...
    try {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        try {
          ...
          dispatchEvent("logError", {
            message: `Audio retry ${i + 1} failed: ${retryErr.message}`,
          });
        }
      }
  ...
  DOM.button1.addEventListener("click", async () => {
    ...
    try {
      ...
      dispatchEvent("logError", {
        message: `Stream toggle error: ${err.message}`,
      });
  ...
  DOM.button2.addEventListener("click", async () => {
    ...
    try {
      ...
      dispatchEvent("logError", {
        message: `Audio toggle error: ${err.message}`,
      });
      await speak("audioError");
    }
  });
