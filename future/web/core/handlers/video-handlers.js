...
export function createVideoHandlers(dispatch) {
  let offscreenCanvas = null;
  let offscreenCtx = null;
  let fpsSamplerInterval = null;
  let frameCount = 0;

  async function startStop({ settingsMode }) {
    const DOM = getDOM();
    try {
      if (settingsMode) {
        // When in settings mode, toggleGrid will be dispatched to the grid handlers
        await dispatch('toggleGrid');
        return; 
      } else {
        if (!settings.stream) {
          let stream;
          ...
          setStream(stream);
          const timerId = setInterval(() => dispatch('processFrame'), settings.updateInterval);
      ...
      await dispatch('updateUI', { settingsMode, streamActive: !!settings.stream, micActive: !!settings.micStream });
    }
  }
  ...
  async function processFrame() {
    ...
    try {
      ...
      if (error) {
        ...
        await dispatch('logError', { message: `Frame processing handler error: ${error.message}` });
        return;
      }
      ...
      await dispatch('logError', { message: `Frame processing error: ${err.message}` });
    }
  }
